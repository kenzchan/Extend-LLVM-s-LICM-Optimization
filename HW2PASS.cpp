...
/// Define by ayou7995
static BasicBlock* GetNextFreqBB(BasicBlock *BB, BranchProbabilityInfo *BPI, BlockFrequencyInfo *BFI) {
	BasicBlock *NextFreqBB = nullptr;
	BranchProbability freqThres = BranchProbability::getBranchProbability(8, 10);
	for (succ_iterator si = succ_begin(BB), sie = succ_end(BB); si != sie; ++si) {
		BranchProbability EdgeProb = BPI->getEdgeProbability(BB, *si);
		if (EdgeProb >= freqThres) NextFreqBB = *si;
	}
	return NextFreqBB;
}
...
bool Correctness::LoopInvariantCodeMotion::runOnLoop(
  ...
  // *****************************************************************
	...
	// 1. Separate all basic blocks into freq and infreq basic blocks.
	BasicBlock *Header = L->getHeader();
	BasicBlock *DummyBlock = Header;
	std::unordered_set<BasicBlock*> LoopBBSet(L->block_begin(), L->block_end());
	std::unordered_set<BasicBlock*> FreqPathBBSet;
	std::unordered_set<BasicBlock*> InfreqPathBBSet(LoopBBSet);

	do {
		FreqPathBBSet.insert(DummyBlock);
		InfreqPathBBSet.erase(DummyBlock);
		DummyBlock = GetNextFreqBB(DummyBlock, BPI, BFI);
	} while (DummyBlock != Header && DummyBlock != nullptr);

	// 2. Find all the store inst in InfreqBBSet
	std::unordered_map<Value*, std::unordered_set<Instruction*> > InfreqStoreInstMap;
	for (BasicBlock* bb: InfreqPathBBSet) {
		for (BasicBlock::iterator i = bb->begin(), ie = bb->end(); i != ie; ++i) {
			if (i->getOpcode() == Instruction::Store) {
				StoreInst *storeInst = (StoreInst *) &(*i);
				Value *operand = storeInst->getOperand(1);
				InfreqStoreInstMap[operand].insert(&(*i));
			}
		}
	}

	// 3. Find all the load inst and store inst in FreqBBSet
	std::unordered_map<Value *, std::unordered_set<Instruction*> > FreqStoreInstMap;
	std::unordered_map<Value *, std::unordered_set<Instruction*> > FreqLoadInstMap;
	for (BasicBlock* bb: FreqPathBBSet) {
		for (BasicBlock::iterator i = bb->begin(), ie = bb->end(); i != ie; ++i) {
			if (i->getOpcode() == Instruction::Store) {
				StoreInst *storeInst = (StoreInst *) &(*i);
				Value *operand = storeInst->getOperand(1);
				FreqStoreInstMap[operand].insert(&(*i));
			} else if (i->getOpcode() == Instruction::Load) {
				LoadInst *loadInst = (LoadInst *) &(*i);
				Value *operand = loadInst->getOperand(0);
				FreqLoadInstMap[operand].insert((&*i));
			}
		}
	}

	// 4. Select the freq load inst that only depend on infreq store but not freq store
	std::unordered_map<Value *, std::unordered_set<Instruction*> > HoistableLoadInstMap;
	for (auto element: FreqLoadInstMap) {
		errs() << "element fist = " << element.first << "\n";
		if (InfreqStoreInstMap.find(element.first) != InfreqStoreInstMap.end() &&
				FreqStoreInstMap.find(element.first) == FreqStoreInstMap.end()) {
			errs() << "i want you " << element.first << " to be hoistable load\n";
			HoistableLoadInstMap[element.first] = element.second;
		}
	}

	// 5. For every same load operand(%i), create a temp (TEMP). 
	// 6. Choose an operand %i, every load instruction should be replaced with a new load inst that takes in TEMP
	// r100 = load(TEMP), and the orignal register (r10) should be replcaed with the new one (r100).
	// 7. Hoist the load inst
	// 8. Create a new store inst in preheader to store TEMP
	// 9. The store inst in Infreq path should be replaced with TEMP
	int sv_cnt = 0; // Stack Variable Count
	int li_cnt = 0; // Load Instruction Count
	for (auto element: HoistableLoadInstMap) {
		int cnt = 0;
		AllocaInst *TEMP;
		bool stored = false;
		for (Instruction *li: element.second) {
			Instruction *nextInst = li->getNextNode();
			BasicBlock *currBB = li->getParent();
			if (cnt == 0)	{
				// Store the first load inst to TEMP
				TEMP = new AllocaInst(li->getType(), 0, 0,
						"STACK_VARIABLE_" + std::to_string(sv_cnt), Preheader->getTerminator());
			}
			// Add new load inst before the hoisted load
			LoadInst* new_LI = new LoadInst(TEMP, "NEW_LOAD_INST_" + std::to_string(li_cnt), nextInst);
			for (BasicBlock::iterator i = currBB->begin(), ie = currBB->end(); i != ie; ++i) {
				Instruction *inst = &(*i);
				inst->replaceUsesOfWith(li, new_LI);
			}
			// Hoist the load inst
			hoist(*li, DT, L, Preheader, &SafetyInfo, MSSAU.get(), ORE);
			// Store inst in preheader
			if (!stored) {
				StoreInst *preheader_SI = new StoreInst(li, TEMP, Preheader->getTerminator());
				stored = true;
			}
			li_cnt++;
			cnt++;
		}
		for (Instruction *si: InfreqStoreInstMap[element.first]) {
			StoreInst *old_si = (StoreInst *) &(*si);
			old_si->setOperand(1, TEMP);
		}
		sv_cnt++;
	}
